...
//! @brief  Definition of Ambient light
/**
 * Ambient light is the type of light that same radiance comes from all directions.
 * This is a simplified version of Sky light.
 */
class   AmbientLight : public SkyLight{
public:
...
protected:
    ...
    //!
    //! WARNING!!!
    //! This function takes advantage of all its usage because the local_dir is merely used to retrieve the ambient light radiance.
    //! Since the radiance is always constant across all directions. There is no need to sample local direction at all, sampling
    ...
    void   SampleLocalDirection(const LightSample& ls, float& pdf_w, Vector& local_dir, Vector& global_dir) const override {
        ...
        local_dir = UniformSampleSphere(ls.u, ls.v);

...
/**
 * Triangle8 is used in QBVH to accelerate ray triangle intersection using AVX. Its sole purpose is to accelerate 
 ...
 */
struct Triangle8{
    __m256  m_p0_x , m_p0_y , m_p0_z ;  /**< Position of point 0 of the triangle. */
    __m256  m_p1_x , m_p1_y , m_p1_z ;  /**< Position of point 1 of the triangle. */
    __m256  m_p2_x , m_p2_y , m_p2_z ;  /**< Position of point 2 of the triangle. */
    __m256  m_mask;

    /**< Pointers to original primitives. */
    const Triangle*  m_ori_tri[8] = { nullptr };
	const Primitive* m_ori_pri[8] = { nullptr };
    ...
    bool PackData(){
        ...
        float	mask[8] = { 1.0f , 1.0f , 1.0f , 1.0f , 1.0f , 1.0f , 1.0f , 1.0f };
        float   p0_x[8] , p0_y[8] , p0_z[8] , p1_x[8] , p1_y[8] , p1_z[8] , p2_x[8] , p2_y[8] , p2_z[8];
        for( auto i = 0 ; i < 8 && m_ori_pri[i] ; ++i ){
            const auto triangle = m_ori_tri[i];
        ...
        m_p0_x = _mm256_set_ps( p0_x[7] , p0_x[6] , p0_x[5] , p0_x[4], p0_x[3] , p0_x[2] , p0_x[1] , p0_x[0] );
        m_p0_y = _mm256_set_ps( p0_y[7] , p0_y[6] , p0_y[5] , p0_y[4], p0_y[3] , p0_y[2] , p0_y[1] , p0_y[0] );
        m_p0_z = _mm256_set_ps( p0_z[7] , p0_z[6] , p0_z[5] , p0_z[4], p0_z[3] , p0_z[2] , p0_z[1] , p0_z[0] );
        m_p1_x = _mm256_set_ps( p1_x[7] , p1_x[6] , p1_x[5] , p1_x[4], p1_x[3] , p1_x[2] , p1_x[1] , p1_x[0] );
        m_p1_y = _mm256_set_ps( p1_y[7] , p1_y[6] , p1_y[5] , p1_y[4], p1_y[3] , p1_y[2] , p1_y[1] , p1_y[0] );
        m_p1_z = _mm256_set_ps( p1_z[7] , p1_z[6] , p1_z[5] , p1_z[4], p1_z[3] , p1_z[2] , p1_z[1] , p1_z[0] );
        m_p2_x = _mm256_set_ps( p2_x[7] , p2_x[6] , p2_x[5] , p2_x[4], p2_x[3] , p2_x[2] , p2_x[1] , p2_x[0] );
        m_p2_y = _mm256_set_ps( p2_y[7] , p2_y[6] , p2_y[5] , p2_y[4], p2_y[3] , p2_y[2] , p2_y[1] , p2_y[0] );
        m_p2_z = _mm256_set_ps( p2_z[7] , p2_z[6] , p2_z[5] , p2_z[4], p2_z[3] , p2_z[2] , p2_z[1] , p2_z[0] );
        m_mask = _mm256_cmpeq_ps( zeros , _mm256_set_ps( mask[7] , mask[6] , mask[5] , mask[4] , mask[3] , mask[2] , mask[1] , mask[0] ) );
...
//!
//! @param	ray			The ray to be tested.
//! @param	tri8		8 Triangles to be tested.
//! @param	quick_quit	Whether to quit as long as an intersection is found, this is usually true for shadow ray.
//! @param	maxt		The maximum distance we are interested. Any intersection behind it is ignored.
//! @param	t8			Output, the distances from ray origin to triangles. It will be FLT_MAX if there is no intersection.
//! @param	u8			Blending factor.
//! @param	v8			Blending factor.
SORT_FORCEINLINE bool intersectTriangle4Inner(const Ray& ray, const Triangle4& tri8, const bool quick_quit, const float maxt, __m256& t8, __m256& u8, __m256& v8, __m256& mask) {
	mask = tri8.m_mask;

	// step 0 : translate the vertices to ray coordinate system
	__m256 p0[3], p1[3], p2[3];
	p0[0] = _mm256_sub_ps(tri4.m_p0_x, ray.m_ori_x);
	p0[1] = _mm256_sub_ps(tri4.m_p0_y, ray.m_ori_y);
	p0[2] = _mm256_sub_ps(tri4.m_p0_z, ray.m_ori_z);

	p1[0] = _mm256_sub_ps(tri4.m_p1_x, ray.m_ori_x);
	p1[1] = _mm256_sub_ps(tri4.m_p1_y, ray.m_ori_y);
	p1[2] = _mm256_sub_ps(tri4.m_p1_z, ray.m_ori_z);

	p2[0] = _mm256_sub_ps(tri4.m_p2_x, ray.m_ori_x);
	p2[1] = _mm256_sub_ps(tri4.m_p2_y, ray.m_ori_y);
	p2[2] = _mm256_sub_ps(tri4.m_p2_z, ray.m_ori_z);

	// step 1 : pick the major axis to avoid dividing by zero in the sheering pass.
	//          by picking the major axis, we can also make sure we sheer as little as possible
	__m256 p0_x = p0[ray.m_local_x];
	__m256 p0_y = p0[ray.m_local_y];
	__m256 p0_z = p0[ray.m_local_z];

	__m256 p1_x = p1[ray.m_local_x];
	__m256 p1_y = p1[ray.m_local_y];
	__m256 p1_z = p1[ray.m_local_z];

	__m256 p2_x = p2[ray.m_local_x];
	__m256 p2_y = p2[ray.m_local_y];
	__m256 p2_z = p2[ray.m_local_z];

	// step 2 : sheer the vertices so that the ray direction points to ( 0 , 1 , 0 )
	p0_x = _mm256_mad_ps(p0_y, ray.m_sse_scale_x, p0_x);
	p0_z = _mm256_mad_ps(p0_y, ray.m_sse_scale_z, p0_z);
	p1_x = _mm256_mad_ps(p1_y, ray.m_sse_scale_x, p1_x);
	p1_z = _mm256_mad_ps(p1_y, ray.m_sse_scale_z, p1_z);
	p2_x = _mm256_mad_ps(p2_y, ray.m_sse_scale_x, p2_x);
	p2_z = _mm256_mad_ps(p2_y, ray.m_sse_scale_z, p2_z);

	// compute the edge functions
	const __m256 e0 = _mm256_sub_ps(_mm256_mul_ps(p1_x, p2_z), _mm256_mul_ps(p1_z, p2_x));
	const __m256 e1 = _mm256_sub_ps(_mm256_mul_ps(p2_x, p0_z), _mm256_mul_ps(p2_z, p0_x));
	const __m256 e2 = _mm256_sub_ps(_mm256_mul_ps(p0_x, p1_z), _mm256_mul_ps(p0_z, p1_x));

	const __m256 c0 = _mm256_and_ps(_mm_and_ps(_mm256_cmpge_ps(e0, zeros), _mm256_cmpge_ps(e1, zeros)), _mm256_cmpge_ps(e2, zeros));
	const __m256 c1 = _mm256_and_ps(_mm_and_ps(_mm256_cmple_ps(e0, zeros), _mm256_cmple_ps(e1, zeros)), _mm256_cmple_ps(e2, zeros));
	mask = _mm256_and_ps(mask, _mm256_or_ps(c0, c1));
	auto c = _mm256_movemask_ps(mask);
	if (0 == c)
		return false;

	const __m256 det = _mm256_add_ps(e0, _mm_add_ps(e1, e2));
	mask = _mm256_and_ps(mask, _mm256_cmpneq_ps(det, zeros));
	c = _mm256_movemask_ps(mask);
	if (0 == c)
		return false;

	const __m256 rcp_det = _mm256_rcpa_ps(det);

	p0_y = _mm256_mul_ps(p0_y, ray.m_sse_scale_y);
	p1_y = _mm256_mul_ps(p1_y, ray.m_sse_scale_y);
	p2_y = _mm256_mul_ps(p2_y, ray.m_sse_scale_y);

	t4 = _mm256_mul_ps(e0, p0_y);
	t4 = _mm256_mad_ps(e1, p1_y, t4);
	t4 = _mm256_mad_ps(e2, p2_y, t4);
	t4 = _mm256_mul_ps(t4, rcp_det);

	const __m256 ray_min_t = _mm256_set_ps1(ray.m_fMin);
	const __m256 ray_max_t = _mm256_set_ps1(ray.m_fMax);
	mask = _mm256_and_ps(_mm256_and_ps(mask, _mm256_cmpgt_ps(t4, ray_min_t)), _mm256_cmple_ps(t4, ray_max_t));
	c = _mm256_movemask_ps(mask);
	if (0 == c)
		return false;
	...
	mask = _mm256_and_ps(_mm256_and_ps(mask, _mm256_cmpgt_ps(t4, zeros)), _mm256_cmplt_ps(t4, _mm256_set_ps1(maxt)));
	c = _mm256_movemask_ps(mask);
	if (0 == c)
		return false;

	// mask out the invalid values
	t4 = _mm256_pick_ps( mask, t4, infinites);

	u4 = _mm256_mul_ps(e1, rcp_det);
	v4 = _mm256_mul_ps(e2, rcp_det);

...
Vector BufferMemory::_genTagentForTri( const std::shared_ptr<Trunk>& trunk , unsigned k ) const
{
	...
	if( determinant == 0.0f )
	{
        slog( LOG_LEVEL::LOG_WARNING , LOG_TYPE::GENERAL , "There is a triangle containing three vertexes with same texture coordinate , can't generate shading coordinate correctly." );

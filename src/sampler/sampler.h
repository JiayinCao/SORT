...
public:
    ...
    virtual void Generate1D( float* sample , unsigned num , RenderContext& rc, bool accept_uniform = false ) const = 0;

    // generate sample in two dimension
    // para 'sample' : the memory to save the sampled data
    // para 'num'    : the number of samples to be generated
    virtual void Generate2D( float* sample , unsigned num , RenderContext& rc, bool accept_uniform = false ) const = 0;
};
